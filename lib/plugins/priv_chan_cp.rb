...
module Cinch
  module Plugins
    class PrivChanCP
      ...
      match /kick (#\S+) (\S+)\s?(.+)?/i, method: :execute_kick
      ...
      match /ban (#\S+) (\S+)/i, method: :execute_ban
      ...
      match /unban (#\S+) (\S+)/i, method: :execute_unban
      ...
      match /kban (#\S+) (\S+)(?: (.+))?/i, method: :execute_kban
      ...
      match /op (#\S+) (\S+)(?: (.+))?/i, method: :execute_rop
      ...
      match /deop (#\S+) (\S+)(?: (.+))?/i, method: :execute_rdop
      ...
      match /voice (#\S+) (\S+)(?: (.+))?/i, method: :execute_rv
      ...
      match /devoice (#\S+) (\S+)(?: (.+))?/i, method: :execute_rdv
      ...
      match /t (#.+?) (.+)/i, method: :execute_topic
      ...
      match /t-append (#.+?) (.+)/i, method: :topic_append

      def topic_append(m, channel, new_topic)
        unless check_master(m.user)
          m.reply Format(:red, "You are not authorized to use this command! This incident will be reported!")
          bot.info("Received invalid topic command from #{m.user.nick}")
          return;
        ...
        c_topic = Channel(channel).topic
        m.reply Format(:green, "Very well...")
        bot.irc.send ("TOPIC #{channel} :#{c_topic} #{new_topic}")
      end
    end
  end
end
