      ...
      module Plugins
        class Weather
          ...
          def current_summary(data, locale)
            ...
            %Q{10Weather: #{locale} | Current Weather: #{data.conditions} | Temp: #{data.temp} °F (#{temp_c} °C) - Feels like: #{data.feels_like} °F (#{feels_temp_c} °C) | Wind: #{data.wind_speed} MPH (#{wind_speed_kph} KPH) - Bearing: #{data.wind_bearing}}
          end
        end
      end
    end
